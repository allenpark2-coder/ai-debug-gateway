...
reserves a board-specific policy extension point for future Felix tools,
but includes no Felix rules until they can be tested on real hardware.
...
policy. An unknown `opsis-*`, Felix, vendor, or locally installed command
is denied by default. Future Felix support will classify exact subcommands
...
- Inferring safety from command names, natural-language purpose, or model
  output.
- Shipping untested Felix command rules.
